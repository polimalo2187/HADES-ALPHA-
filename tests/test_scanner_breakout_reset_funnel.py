...
def test_breakout_reset_funnel_maps_debug_lifecycle_counts():
    funnel = _build_breakout_reset_funnel(
        reject_totals={
            ...
            "breakout_reset_near_miss": 14,
            "breakout_setup_expired": 1,
            "stateful_setup_back_inside_range": 2,
            "breakout_invalidated_recent": 5,
            "breakout_drifted_back_inside_recent": 6,
            "stateful_setup_invalid_price": 1,
    ...
    assert funnel["reset_near_miss"] == 14
    assert funnel["reset_touched_candidates"] == 9
    assert funnel["published_selected"] == 1
    assert funnel["expired_no_reset"] == 1
    assert funnel["invalidated"] == 14
    assert funnel["invalidated_back_inside"] == 2
    assert funnel["invalidated_breakout_invalidated"] == 5
    assert funnel["invalidated_breakout_back_inside"] == 6
    assert funnel["invalidated_invalid_price"] == 1
    assert funnel["soft_gate_hits"] == 3
    assert funnel["score_floor_rejected"] == 4
    assert funnel["hard_gate_rejected"] == 18
