...
class AccountCenterPayloadTests(unittest.TestCase):
    def test_build_account_center_payload_includes_subscription_billing_and_referrals(self):
        ...
        with patch('app.miniapp.service.build_watchlist_context', return_value={'meta': {'symbols': ['BTCUSDT'], 'symbols_count': 1, 'max_symbols': 25, 'slots_left': 24, 'plan': 'premium', 'plan_name': 'PREMIUM', 'can_add_more': True}}),              patch('app.miniapp.service.get_active_payment_order_for_user', return_value=active_order),              patch('app.miniapp.service._load_recent_payment_orders', return_value=recent_orders),              patch('app.miniapp.service._load_payment_order_summary', return_value={'open': 1, 'completed': 2, 'expired': 0, 'cancelled': 1, 'total': 4}),              patch('app.miniapp.service.get_user_referral_stats', return_value=referral_stats),              patch('app.miniapp.service.get_referral_link', return_value='https://t.me/HADES_ALPHA_bot?start=ref_42'),              patch('app.miniapp.service._load_recent_referral_rewards', return_value=referral_rewards),              patch('app.miniapp.service._load_recent_subscription_events', return_value=timeline),              patch('app.miniapp.service.get_payment_configuration_status', return_value={'ready': True, 'checks': [{'key': 'BSC_RPC_HTTP_URL', 'label': 'RPC BSC', 'value_present': True}, {'key': 'PAYMENT_TOKEN_CONTRACT', 'label': 'Contrato del token', 'value_present': True}, {'key': 'PAYMENT_RECEIVER_ADDRESS', 'label': 'Wallet receptora', 'value_present': True}], 'missing_keys': []}),              patch('app.miniapp.service.plan_status', return_value={'plan': 'premium', 'status': 'active', 'expires': None, 'days_left': 12}):
    ...
    def test_bootstrap_payload_stays_light_and_keeps_root_runtime_data(self):
        user = {
            'user_id': 11,
            'username': 'neo',
            'language': 'es',
        ...
        with patch('app.miniapp.service.get_bot_username', return_value='NeoTrade_bot'),              patch('app.miniapp.service.get_watchlist_limit_for_plan', return_value=2):
        ...
        self.assertEqual(payload['watchlist_meta']['max_symbols'], 2)
        self.assertEqual(payload['account'], {})
        self.assertEqual(payload['plans'], {'plus': [], 'premium': []})
        self.assertNotIn('payment_config_status', payload)
        self.assertNotIn('payment_config_ready', payload)
