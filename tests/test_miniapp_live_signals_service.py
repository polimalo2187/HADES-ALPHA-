...
from app.miniapp.service import build_live_signals_feed_meta, build_live_signals_payload


class MiniAppLiveSignalsServiceTests(unittest.TestCase):
    def test_live_signals_feed_meta_exposes_feed_version_without_serializing_payload(self):
        now = datetime.utcnow()
        active_docs = [
            {'created_at': now, 'updated_at': now, 'telegram_valid_until': now + timedelta(minutes=10)},
            {'created_at': now - timedelta(minutes=1), 'updated_at': now - timedelta(minutes=1), 'telegram_valid_until': now + timedelta(minutes=5)},
        ]
        recent_docs = active_docs + [
            {'created_at': now - timedelta(minutes=5), 'updated_at': now - timedelta(minutes=3), 'telegram_valid_until': now - timedelta(minutes=1)},
        ]
        ...
        with patch('app.miniapp.service.user_signals_collection', return_value=collection):
            payload = build_live_signals_feed_meta({'user_id': 55})
    ...
    def test_live_signals_payload_reuses_precomputed_meta(self):
        now = datetime.utcnow()
        active_docs = [
        ...
        meta = {
            ...
            'latest_signal_activity_at': now.isoformat(),
            'latest_active_signal_activity_at': now.isoformat(),
            'feed_version': '2|meta',
            'generated_at': now.isoformat(),
        }

        with patch('app.miniapp.service.user_signals_collection', return_value=collection):
            payload = build_live_signals_payload({'user_id': 55}, meta=meta)
        ...
        self.assertEqual(payload['feed_version'], '2|meta')
        self.assertEqual(collection.count_documents.call_count, 0)
