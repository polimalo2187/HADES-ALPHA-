...
class MiniAppAuthEndpointTests(unittest.TestCase):
    ...
    def test_dev_auth_is_rejected_when_disabled(self):
        with patch('app.miniapp.app.initialize_database'),              patch('app.miniapp.app.start_background_heartbeat'),              patch('app.miniapp.app.get_mini_app_cors_origins', return_value=['https://hades.example.com']),              patch('app.miniapp.app.is_mini_app_dev_auth_enabled', return_value=False),              patch('app.miniapp.app.get_runtime_role', return_value='web'):
    ...
    def test_dev_auth_rejects_unconfigured_user_id_mismatch(self):
        ...
        with patch('app.miniapp.app.initialize_database'),              patch('app.miniapp.app.start_background_heartbeat'),              patch('app.miniapp.app.get_mini_app_cors_origins', return_value=['https://hades.example.com']),              patch('app.miniapp.app.is_mini_app_dev_auth_enabled', return_value=True),              patch('app.miniapp.app.get_mini_app_dev_user_id', return_value=123),              patch('app.miniapp.app.get_runtime_role', return_value='web'),              patch('app.miniapp.app.ensure_mini_app_user', return_value=fake_user),              patch('app.miniapp.app.build_me_payload', return_value={'user_id': 123, 'is_admin': True}):
            with self._build_client() as client:
        ...
        self.assertTrue(payload['me']['is_admin'])
