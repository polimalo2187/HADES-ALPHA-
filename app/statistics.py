...
def _r_multiple_value(result_doc: Dict[str, Any]) -> Optional[float]:
    ...
    if outcome == "won":
        ...
        # Fallback legacy conservador:
        # si la fila antigua ya cuenta como WON pero no trae resolución ni r_multiple,
        # la modelamos como TP1 = +1R para no contaminar PF/Expectancy con 0R falsos.
        return 1.0
...
def get_materialized_window(days: int, *, max_age_seconds: int = MATERIALIZED_MAX_AGE_SECONDS) -> Optional[Dict[str, Any]]:
    ...
    if schema_version != STATS_SNAPSHOT_SCHEMA_VERSION:
        return None
