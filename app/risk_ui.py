...
def _history_result_label(doc: Dict, language: str | None = "es") -> str:
    result = str(doc.get("result") or "").lower()
    mapping_es = {"won": "GANADA", "lost": "PERDIDA", "expired": "EXPIRADA"}
    mapping_en = {"won": "WON", "lost": "LOST", "expired": "EXPIRED"}
    mapping = mapping_en if _lang(language) == "en" else mapping_es
    if result in mapping:
        return mapping[result]
    return _signal_status(doc, language)


def build_history_list_text(docs: Iterable[Dict], *, language: str = "es") -> str:
    docs = list(docs)
    lines = [_t(language, "🧾 HISTORIAL VERIFICABLE", "🧾 VERIFIED HISTORY"), "", _t(language, "Selecciona una señal cerrada para ver detalle, análisis y cálculo de riesgo:", "Select a closed signal to view details, analysis, and risk calculation:"), ""]
    for idx, d in enumerate(docs, 1):
        score = d.get("normalized_score") if d.get("normalized_score") is not None else d.get("score")
        score_label = f" | Score {float(score):.1f}" if score is not None else ""
        lines.append(f"{idx}. {d.get('symbol', '—')} | {d.get('direction', '—')} | {_history_result_label(d, language)}{score_label}")
    return "\n".join(lines).strip()
...
def build_history_list_keyboard(docs: Iterable[Dict], *, language: str = "es") -> InlineKeyboardMarkup:
    ...
    for signal in docs:
        signal_id = signal.get("signal_id") or str(signal.get("_id") or "")
        result_label = _history_result_label(signal, language)
        label = f"{signal.get('symbol', '—')} {signal.get('direction', '—')} · {result_label}"
        rows.append([InlineKeyboardButton(label[:64], callback_data=f"hist_detail:{signal_id}")])
