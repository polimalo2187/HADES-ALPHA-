...
"🏠 MENÚ PRINCIPAL — Selecciona una opción abajo",
        ...
        if action in ["choose_plus_plan_days", "choose_premium_plan_days"]:
            ...
            if not target_user_id:
                await query.edit_message_text("❌ No hay un usuario seleccionado.")
                return
            ...
            await query.edit_message_text(
                f"📅 Envía la cantidad de días para activar el PLAN {plan_name} del usuario {target_user_id}."
            )
            return
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        or context.user_data.get("awaiting_plan_days")
    ):
    ...
    try:
        ...
        if context.user_data.get("awaiting_plan_days"):
            try:
                ...
                await update.message.reply_text("❌ Cantidad de días inválida. Debe ser un número entero.")
                return
