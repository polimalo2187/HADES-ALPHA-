...
from app.database import sentinel_link_tokens_collection
...
class SentinelBridgeError(ValueError):
    """Error controlado para el puente HADES → Sentinel."""


DEFAULT_SENTINEL_URL = "https://hades-sentinel-production.up.railway.app"
...
def _sentinel_url() -> str:
    url = (
        ...
        or DEFAULT_SENTINEL_URL
    ).rstrip("/")
    if not (url.startswith("https://") or url.startswith("http://")):
        raise SentinelBridgeError("SENTINEL_URL inválido")
    return url


def _link_ttl_seconds() -> int:
    try:
        return max(30, min(int(os.getenv("SENTINEL_LINK_TTL_SECONDS", "60") or "60"), 300))
    except Exception:
        return 60
...
def build_sentinel_entitlement(user: Dict[str, Any]) -> Dict[str, Any]:
...
def create_sentinel_link(user: Dict[str, Any], *, request_id: Optional[str] = None) -> Dict[str, Any]:
    entitlement = build_sentinel_entitlement(user)
    if not entitlement.get("ok"):
        raise SentinelBridgeError("premium_required")
    ...
    sentinel_link_tokens_collection().insert_one({
    ...
    url = f"{_sentinel_url()}/auth/hades/link?{urlencode({'code': code})}"
    return {
        "ok": True,
        "url": url,
