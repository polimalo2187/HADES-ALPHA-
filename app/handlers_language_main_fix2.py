...
from app.menus import main_menu, back_to_menu, get_main_menu_header, normalize_language
...
def _user_language(user: dict | None, telegram_language: str | None = None) -> str:
    if user and user.get("language"):
        return normalize_language(user.get("language"))
    return normalize_language(telegram_language)


def _main_menu_text(language: str) -> str:
    return get_main_menu_header(language)
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        tg_lang = normalize_language(getattr(update.effective_user, "language_code", None))
        ...
        if not user:
            ...
            if not user.get("language"):
                users_col.update_one({"user_id": user_id}, {"$set": {"language": tg_lang}})

        current_user = users_col.find_one({"user_id": user_id})
        lang = _user_language(current_user, tg_lang)
        await update.message.reply_text(
            _main_menu_text(lang),
            reply_markup=main_menu(is_admin=is_admin(user_id), language=lang),
        )
    ...
    try:
        ...
        if not user:
            await query.edit_message_text(
                ("User not found. Please use /start again." if _user_language(None, getattr(query.from_user, "language_code", None)) == "en" else "Usuario no encontrado. Usa /start nuevamente."),
                reply_markup=main_menu(language=_user_language(None, getattr(query.from_user, "language_code", None))),
            )
            return
        ...
        if action == "back_menu":
            ...
            lang = _user_language(user, getattr(query.from_user, "language_code", None))
            await query.edit_message_text(
                _main_menu_text(lang),
                reply_markup=main_menu(is_admin=admin, language=lang),
            )
            return
        ...
        await query.edit_message_text(
            ("❌ An unexpected error occurred." if _user_language(user if "user" in locals() else None, getattr(query.from_user, "language_code", None)) == "en" else "❌ Ocurrió un error inesperado."),
            reply_markup=main_menu(language=_user_language(user if "user" in locals() else None, getattr(query.from_user, "language_code", None))),
        )
...
async def handle_copy_ref_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ("❌ Error copying link." if _user_language(users_collection().find_one({"user_id": update.effective_user.id}), getattr(update.effective_user, "language_code", None)) == "en" else "❌ Error al copiar enlace."),
            reply_markup=main_menu(language=_user_language(users_collection().find_one({"user_id": update.effective_user.id}), getattr(update.effective_user, "language_code", None))),
        )
...
async def handle_exchange_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(
            (f"✅ Exchange confirmed: {exchange_name}\nMain menu:" if _user_language(users_collection().find_one({"user_id": update.effective_user.id}), getattr(update.effective_user, "language_code", None)) == "en" else f"✅ Exchange confirmado: {exchange_name}\nMenú principal:"),
            reply_markup=main_menu(language=_user_language(users_collection().find_one({"user_id": update.effective_user.id}), getattr(update.effective_user, "language_code", None))),
        )
