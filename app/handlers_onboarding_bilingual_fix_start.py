...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Maneja /start, captura referidos y muestra onboarding bilingüe cuando corresponda."""
    try:
        ...
        if not user:
            ...
            if "onboarding_seen" not in user:
                updates["onboarding_seen"] = False
            if "onboarding_completed" not in user:
                updates["onboarding_completed"] = False
            if "onboarding_version" not in user:
                updates["onboarding_version"] = 0
            if updates:
