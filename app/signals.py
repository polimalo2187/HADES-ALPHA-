...
def _base_signal_from_history(history_doc: Dict, signal_id: str) -> Dict:
    ...
    return {
        ...
        "send_mode": history_doc.get("send_mode"),
        "strategy_name": history_doc.get("strategy_name"),
        "strategy_version": history_doc.get("strategy_version"),
        "regime_state": history_doc.get("regime_state"),
        "regime_reason": history_doc.get("regime_reason"),
        "evaluated": True,
...
def _strategy_tracking_meta(strategy_name: Optional[str], send_mode: Optional[str] = None) -> Dict[str, str]:
...
def _build_tracking_live_summary(
    *,
    strategy_meta: Dict[str, str],
    ...
    in_entry_zone: bool,
    signal_active_for_entry: bool,
    evaluation_window_open: bool,
    entry_state_label: str,
    tracking_copy: Dict[str, str],
) -> tuple[str, str]:
    key = strategy_meta.get("key")
    ...
    return (
        "La señal ya no está operativa. Úsala solo como referencia histórica.",
        "Solo historial",
    )
    ...
    for s in pending:
        try:
            ...
            result_doc = new_signal_result(
                ...
                regime_reason=s.get("regime_reason"),
                result=result,
