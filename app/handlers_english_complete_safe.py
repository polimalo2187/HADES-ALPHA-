...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if user and user.get("banned"):
            await update.message.reply_text(_tr(_get_user_language(user), "🚫 Tu acceso al bot ha sido revocado.", "🚫 Your access to the bot has been revoked."))
            return
        ...
        if is_new_user:
            doc = {
                "user_id": user_id,
                "plan": PLAN_FREE,
                "language": normalize_language(getattr(update.effective_user, "language_code", None)),
                "ref_plus_valid": 0,
                "ref_premium_valid": 0,
                "ref_plus_total": 0,
                "ref_premium_total": 0,
        ...
        await update.message.reply_text(
            _tr(
                language,
                "🏠 MENÚ PRINCIPAL\n\nBienvenido a HADES ALPHA V2.\nAquí puedes acceder a señales en vivo, análisis, rendimiento, mercado y herramientas del bot.\n\nSelecciona una opción abajo:",
                "🏠 MAIN MENU\n\nWelcome to HADES ALPHA V2.\nHere you can access live signals, analysis, performance, market data, and bot tools.\n\nSelect an option below:",
            ),
        ...
        await update.message.reply_text("❌ Error starting the bot.")
...
async def handle_view_signals(query, user, admin, users_col):
    try:
        ...
        if not user_signals:
            await query.edit_message_text(
                _tr(_get_user_language(user), "📭 No hay señales activas disponibles.", "📭 There are no active signals available."),
                reply_markup=back_to_menu(language=language),
            )
            return

        await query.edit_message_text(
            build_active_signals_list_text(user_signals, language=_get_user_language(user)),
            reply_markup=build_active_signals_list_keyboard(user_signals, language=_get_user_language(user)),
        )
        ...
        await query.edit_message_text(
            _tr(_get_user_language(user), "❌ Error al obtener señales.", "❌ Error while loading signals."),
            reply_markup=back_to_menu(language=language),
        )
...
async def handle_history(query, user):
    ...
    try:
        ...
        if not docs:
            try:
                ...
                if "Message is not modified" in str(e):
                    await query.answer(_tr(_get_user_language(user), "✅ Actualizado", "✅ Updated"), show_alert=False)
                else:
                    raise
            return
        ...
        try:
            await query.edit_message_text(
                build_history_list_text(docs, language=_get_user_language(user)),
                reply_markup=build_history_list_keyboard(docs, language=_get_user_language(user)),
            )
        except BadRequest as e:
            if "Message is not modified" in str(e):
                await query.answer(_tr(_get_user_language(user), "✅ Actualizado", "✅ Updated"), show_alert=False)
