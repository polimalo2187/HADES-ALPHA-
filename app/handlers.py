...
async def handle_market(query, user):
    ...
    try:
        text, keyboard = render_market_state(plan=(user.get("plan") or PLAN_FREE), language=_get_user_language(user))
        ...
        await query.edit_message_text(
            _tr(_get_user_language(user), "⚠️ No pude cargar Mercado ahora mismo. Intenta de nuevo en unos segundos.", "⚠️ I could not load Market right now. Try again in a few seconds."),
