...
from app.signals import get_latest_base_signal_for_plan, format_user_signal, get_user_signal_by_signal_id, get_recent_user_signals_for_user, get_signal_analysis_for_user
...
async def _show_signal_detail(query, user, signal_id: str, source: str = "live"):
    ...
    await query.edit_message_text(
        ...
        reply_markup=build_signal_detail_keyboard(signal_id, source=source),
    )


async def _show_signal_analysis(query, user, signal_id: str, source: str = "live"):
    profile_name = get_user_risk_profile(user["user_id"]).get("default_profile")
...
async def _show_risk_result(query, user, signal_id: str, source: str = "live", profile_name: str | None = None):
    ...
    await query.edit_message_text(
        build_risk_result_text(calc),
        reply_markup=build_risk_result_keyboard(signal_id, source=source),
    )
...
async def _handle_risk_dynamic_callbacks(query, context, user, action: str) -> bool:
    ...
    if action == "risk_set_profile":
        ...
        await query.edit_message_text(
            ...
            reply_markup=build_default_profile_selection_keyboard(profile.get("default_profile")),
        )
        return True
    ...
    if action.startswith("risk_pf:"):
        ...
        current = get_user_risk_profile(user_id)
        await query.edit_message_text(
            build_signal_profile_picker_text(current.get("default_profile"), source=source),
            reply_markup=build_signal_profile_picker_keyboard(signal_id, source=source, selected_profile=current.get("default_profile")),
        )
        return True
