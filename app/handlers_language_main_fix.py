...
from app.menus import main_menu, back_to_menu, normalize_language, get_menu_text
...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        language = normalize_language((users_col.find_one({"user_id": user_id}) or {}).get("language") or getattr(update.effective_user, "language_code", None))
        await update.message.reply_text(
            get_menu_text(language, is_admin=is_admin(user_id)),
            reply_markup=main_menu(is_admin=is_admin(user_id), language=language),
        )
    ...
    try:
        ...
        if not user:
            await query.edit_message_text(
                "User not found. Use /start again." if normalize_language(getattr(query.from_user, "language_code", None)) == "en" else "Usuario no encontrado. Usa /start nuevamente.",
                reply_markup=main_menu(language=normalize_language(getattr(query.from_user, "language_code", None))),
            )
            return
        ...
        if action == "back_menu":
            ...
            language = normalize_language((user.get("language") if user else None) or getattr(query.from_user, "language_code", None))
            await query.edit_message_text(
                get_menu_text(language, is_admin=admin),
                reply_markup=main_menu(is_admin=admin, language=language),
            )
            return
